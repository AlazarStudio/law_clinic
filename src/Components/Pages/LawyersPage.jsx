...
const LawyersPage = () => {
    ...
    const fetchLawyers = () => {
        fetch("https://law-clinik-back.onrender.com/users")
            .then(res => res.json())
            .then(data => setLawyers(data.filter(u => u.role === "lawyer")));
    };
    ...
    const getContractStats = (userId) => {
        const userContracts = contracts.filter((c) => c.userId === userId);
        return {
            total: userContracts.length,
            signed: userContracts.filter((c) => c.status === "Подписан").length,
            pending: userContracts.filter((c) => c.status === "На согласовании").length,
        };
    };

    return (
        <Container sx={{ mt: 4 }}>
            <Typography variant="h4" gutterBottom>Список юристов</Typography>
            <Paper elevation={3}>
                <Table>
                    <TableHead>
                        <TableRow>
            ...
            {editUser && (
                <Dialog open={true} onClose={() => setEditUser(null)}>
                    <DialogTitle>Редактировать юриста</DialogTitle>
